...
test('public release identity is internally consistent', async () => {
  ...
  assert.match(client, /const SKILL_VERSION = '1\.0\.2'/u)
  assert.match(client, /COMPATIBLE_STATE_SKILL_VERSIONS = new Set\(\['1\.0\.0', '1\.0\.1', SKILL_VERSION\]\)/u)
  assert.match(readme, /immutable release `porta-product-lifecycle-v1\.0\.2`/u)
  assert.match(readme, /git clone --branch porta-product-lifecycle-v1\.0\.2/u)
