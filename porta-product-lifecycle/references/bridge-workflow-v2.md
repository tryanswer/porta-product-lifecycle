# Porta Product Lifecycle Bridge v2
...
does not need to name the Skill or use `$porta-product-lifecycle`; context can establish
...
directly after installation without activating the Porta Product Lifecycle Skill or
...
```json
{
  ...
  "installedSkills": [
    { "id": "porta-product-lifecycle", "path": "porta-product-lifecycle" }
  ],
  "provider": "codex",
  ...
  "release": {
    ...
    "tag": "porta-product-lifecycle-v1.0.4",
    "version": "1.0.4"
  },
...
```bash
node "/absolute/path/to/porta-product-lifecycle/scripts/porta-product-lifecycle.mjs" \
  capabilities --workflow-protocol-version 2
node "/absolute/path/to/porta-product-lifecycle/scripts/porta-product-lifecycle.mjs" new-run-key
```
