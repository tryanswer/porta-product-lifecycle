...
Use this local-first Adapter when the current Agent needs to hand one generated
file to the same Porta user on the phone. Porta retains one Inbox record, may
show an immediate Terminal popup, verifies the exact bytes before preview or
save, and stores a user-requested download on the device.

This is presentation only. It does not create a product package, lifecycle run,
deployment, public link, cloud artifact, or cross-user message.
...
1. Establish the current request id, Provider session id, project root, and
   ...
   session, Host, or file by guesswork. One file uses one unique request id.
   Never reuse a request id for a different file, bytes, Provider/session,
   title, intent, or turn.
...
4. Use the bundled `publish` command with the exact cwd, file, request,
...
- A missing or incompatible Bridge, escaped path, changed bytes, expired
  record, full retention capacity, malformed receipt, or unsupported Provider
  fails closed. Do not fall back to embedding binary data in chat or publishing
  a public URL.
