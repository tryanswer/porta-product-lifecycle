...
const MINIMUM_PRODUCT_MATERIALIZATION_RUNTIME = '1.18.0'
const MINIMUM_LOCAL_PRODUCT_RELEASE_RUNTIME = '1.18.0'
...
async function main() {
  ...
  if (command === '--version' || command === 'version') {
    writeResult({ ok: true, skillId: SKILL_ID, skillVersion: SKILL_VERSION, type: 'porta-product-lifecycle-client-version' })
    return
  }
  if (command === 'new-run-key') {
    requireNoArguments(tokens)
    writeResult({ ok: true, runKey: `run_${randomUUID()}`, type: 'porta-product-lifecycle-run-key' })
    return
  }
  if (command === 'capabilities') {
    ...
    writeResult({ capabilities, ok: true, type: 'porta-product-lifecycle-client-capabilities' })
    return
  }
  ...
  if (command === 'package-validate') {
    ...
    writeResult({
      ...
      skillId: SKILL_ID,
      skillVersion: SKILL_VERSION,
      type: 'porta-product-lifecycle-product-package',
    })
    return
  }
  if (command === 'capability-negotiate') {
    const options = parseOptions(tokens, ['spec'])
    const result = await readAndNegotiateProductCapabilities(options.spec)
    writeResult({
      ...result,
      ok: true,
      skillId: SKILL_ID,
  ...
  if (command === 'package-verify') {
    ...
    const receipt = await verifyProductPackageRoot(descriptor.package, options['package-root'])
    writeResult({
      ok: true,
      receipt,
      skillId: SKILL_ID,
      skillVersion: SKILL_VERSION,
      type: 'porta-product-lifecycle-product-package-verification',
    })
    return
  }
  if (command === 'lifecycle-plan') {
    const options = parseOptions(tokens, ['spec'])
    const plan = await readAndPlanProductLifecycle(options.spec)
    writeResult({
      ...plan,
      ok: true,
      skillId: SKILL_ID,
  ...
  if (command === 'local-release-status') {
    await readLocalProductReleaseStatus(tokens)
    return
  }
  ...
  if (command === 'private-product-status') {
    await readPrivateProductStatus(tokens)
    return
  }
  if (command === 'begin') {
    await begin(tokens)
...
async function registerProductMaterialization(tokens, operationKind, routeReceipt) {
  ...
  const packageRoot = await resolveExistingDirectory(options['package-root'], 'Product Package root')
  const specPath = await resolveExistingFile(options.spec, 'Product Package descriptor')
  const descriptor = await readProductPackage(specPath)
  ...
  const identity = {
    artifactSha256: receipt.materializationCandidate.primaryArtifact.sha256,
  ...
  writeResult({
    complete: false,
    ok: true,
    requestRef: state.registrationReceipt.requestRef,
    stateFile,
    status: state.registrationReceipt.status,
    type: `porta-product-lifecycle-${operationKind}-registration`,
    workRunKind: 'non-publish-product-work',
  })
}

async function readLocalProductReleaseStatus(tokens) {
  return readProductMaterializationStatus(tokens, 'local-release')
}

async function readPrivateProductStatus(tokens) {
  return readProductMaterializationStatus(tokens, 'private-product')
}
...
async function readProductMaterializationStatus(tokens, operationKind) {
  ...
  if (!state?.registrationReceipt) {
    throw new ClientError(
      `${operationKind.replaceAll('-', '_')}_state_missing`,
      `No registered ${operationKind === 'local-release' ? 'Local Product Release' : 'Private Product'} operation exists for this Run key.`,
    )
  }
  const traceId = `porta-${operationKind}-status:${randomUUID()}`
  const value = await runBridge([
    'workflow', 'product-materialization-status',
    '--workflow-protocol-version', '2',
    '--request-ref', state.registrationReceipt.requestRef,
    '--trace-id', traceId,
    '--json',
  ])
  const status = operationKind === 'local-release'
    ? normalizeLocalProductReleaseStatus(value, state, traceId)
    : normalizePrivateProductStatus(value, state, traceId)
  const completionStatus = operationKind === 'local-release' ? 'local-ready' : 'ready'
  writeResult({
    complete: status.status === completionStatus,
    ok: true,
    requestRef: status.requestRef,
    ...(status.status === completionStatus ? { receipt: status } : {}),
    stateFile,
    status: status.status,
    type: `porta-product-lifecycle-${operationKind}-status`,
  })
}
...
async function requireProductMaterializationCapabilities(operationKind) {
  ...
  const requiredCommands = [
    'product-materialization-register',
    'product-materialization-status',
    'product-work-begin',
  ]
  if (!isRecord(capabilities) || capabilities.ok !== true || capabilities.protocolVersion !== 1 ||
...
function requireLocalProductPackage(candidate) {
  const target = candidate?.package?.deploymentTarget
  const profile = candidate?.package?.profile
  if (target?.placement !== 'local-machine' || !['loopback', 'private'].includes(target.exposure) ||
    !['static-web', 'local-runtime', 'mobile'].includes(profile?.kind)) {
    throw new ClientError(
      'local_release_target_invalid',
      'Local Product Release requires a verified local-machine loopback/private Product Package.',
    )
  }
}

function requirePrivateProductPackage(candidate) {
  const target = candidate?.package?.deploymentTarget
  const productPackage = candidate?.package
  if (productPackage?.profile?.kind !== 'static-web' ||
    productPackage?.distribution !== undefined ||
    target?.exposure === 'public' || target?.placement === 'local-machine') {
    throw new ClientError(
      'private_product_target_invalid',
      'Private Product requires a verified static-web Product Package without local-machine, public exposure, or Distribution intent.',
    )
  }
}

function normalizeLocalProductWorkReceipt(value, state, traceId) {
  const receipt = requireExactRecord(value, [
    'created', 'ok', 'projectContext', 'protocolVersion', 'provider',
    'providerSessionId', 'purpose', 'status', 'traceId', 'type', 'version',
    'workflowProtocolVersion', 'workRunId',
  ], [], 'Product Work begin receipt')
  if (receipt.ok !== true || receipt.protocolVersion !== 1 || receipt.workflowProtocolVersion !== 2 ||
    receipt.version !== 1 || receipt.type !== 'product-work-begin' || receipt.traceId !== traceId ||
    typeof receipt.created !== 'boolean' || receipt.provider !== state.provider ||
    receipt.providerSessionId !== state.providerSessionId || receipt.purpose !== 'materialization' ||
    receipt.status !== 'active' || !workflowRunPattern.test(String(receipt.workRunId ?? '')) ||
    !isRecord(receipt.projectContext) || receipt.projectContext.rootPath !== state.cwd ||
    !Number.isSafeInteger(receipt.projectContext.generation) || receipt.projectContext.generation < 1 ||
    !isWorkflowOpaqueRef(receipt.projectContext.projectRef)) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge returned an invalid non-publish Product Work receipt.')
  }
  return receipt
}

function normalizeLocalProductRegistrationReceipt(value, traceId) {
  const receipt = requireExactRecord(value, [
    'idempotent', 'ok', 'protocolVersion', 'requestRef', 'status', 'traceId',
    'type', 'version', 'workflowProtocolVersion',
  ], [], 'Product Materialization registration receipt')
  if (receipt.ok !== true || receipt.protocolVersion !== 1 || receipt.workflowProtocolVersion !== 2 ||
    receipt.version !== 1 || receipt.type !== 'product-materialization-registration-receipt' ||
    receipt.traceId !== traceId || typeof receipt.idempotent !== 'boolean' ||
    receipt.status !== 'pending' || !/^materialization_[A-Za-z0-9._:-]{8,159}$/.test(String(receipt.requestRef ?? ''))) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge returned an invalid Product Materialization registration receipt.')
  }
  return receipt
}

function normalizeLocalProductReleaseStatus(value, state, traceId) {
  if (!isRecord(value) || value.ok !== true || value.protocolVersion !== 1 ||
    value.workflowProtocolVersion !== 2 || value.version !== 1 ||
    value.type !== 'product-materialization-status' || value.traceId !== traceId ||
    value.requestRef !== state.registrationReceipt.requestRef ||
    !['pending', 'recovery', 'local-ready'].includes(value.status)) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge returned an invalid Local Product Release status.')
  }
  if (value.status !== 'local-ready') {
    return requireExactRecord(value, [
      'ok', 'protocolVersion', 'requestRef', 'status', 'traceId', 'type',
      'version', 'workflowProtocolVersion',
    ], [], 'Pending Local Product Release status')
  }
  const receipt = requireExactRecord(value, [
    'adapter', 'artifactSha256', 'custodyRef', 'ok', 'operationRef',
    'packageDigest', 'protocolVersion', 'receiptRef', 'requestRef', 'settledAt',
    'status', 'targetRef', 'traceId', 'type', 'version', 'workflowProtocolVersion',
  ], [], 'Local Product Release ready receipt')
  if (!['android', 'ios', 'local-runtime', 'static-web'].includes(receipt.adapter) ||
    receipt.artifactSha256 !== state.artifactSha256 || receipt.packageDigest !== state.packageDigest ||
    !isWorkflowOpaqueRef(receipt.custodyRef) || !isWorkflowOpaqueRef(receipt.receiptRef) ||
    !isWorkflowOpaqueRef(receipt.targetRef) || !isIsoDateTime(receipt.settledAt) ||
    !/^local-release-op:(?:deploy|rollback):[a-f0-9]{64}:(?:none|[a-f0-9]{64}):[a-f0-9]{32}$/.test(String(receipt.operationRef ?? ''))) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge Local Product Release identity changed.')
  }
  return receipt
}

function normalizePrivateProductStatus(value, state, traceId) {
  if (!isRecord(value) || value.ok !== true || value.protocolVersion !== 1 ||
    value.workflowProtocolVersion !== 2 || value.version !== 1 ||
    value.type !== 'product-materialization-status' || value.traceId !== traceId ||
    value.requestRef !== state.registrationReceipt.requestRef ||
    !['pending', 'recovery', 'ready'].includes(value.status)) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge returned an invalid Private Product status.')
  }
  if (value.status !== 'ready') {
    return requireExactRecord(value, [
      'ok', 'protocolVersion', 'requestRef', 'status', 'traceId', 'type',
      'version', 'workflowProtocolVersion',
    ], [], 'Pending Private Product status')
  }
  const receipt = requireExactRecord(value, [
    'contentDigest', 'latestReadyRevisionRef', 'ok', 'productRef',
    'productVersion', 'protocolVersion', 'readyAt', 'requestRef', 'revisionRef',
    'status', 'traceId', 'type', 'version', 'workflowProtocolVersion',
  ], [], 'Private Product ready receipt')
  if (![receipt.contentDigest, state.packageDigest].every((digest) => /^[a-f0-9]{64}$/.test(String(digest ?? ''))) ||
    !isWorkflowOpaqueRef(receipt.productRef) || !isWorkflowOpaqueRef(receipt.revisionRef) ||
    receipt.latestReadyRevisionRef !== receipt.revisionRef ||
    !Number.isSafeInteger(receipt.productVersion) || receipt.productVersion < 1 ||
    !isIsoDateTime(receipt.readyAt)) {
    throw new ClientError('malformed_bridge_receipt', 'Agent Bridge Private Product identity is invalid.')
  }
  return receipt
}

async function productMaterializationStateFile(runKey, operationKind) {
  const home = await realpath(homedir()).catch(() => undefined)
  if (!home) throw new ClientError('product_materialization_state_unavailable', 'User home is unavailable for Product Materialization recovery state.')
  const porta = join(home, '.porta')
  const lifecycle = join(porta, 'lifecycle-client')
  const operations = join(lifecycle, operationKind)
  for (const directory of [porta, lifecycle, operations]) await ensureSafeDirectory(directory)
  await chmod(operations, 0o700).catch(() => undefined)
  return join(operations, `${runKey}.json`)
}

async function productMaterializationCandidateFile(cwd, runKey, operationKind) {
  const porta = join(cwd, '.porta')
  const lifecycle = join(porta, 'lifecycle-client')
  const operations = join(lifecycle, operationKind)
  for (const directory of [porta, lifecycle, operations]) await ensureSafeDirectory(directory)
  await chmod(operations, 0o700).catch(() => undefined)
  return join(operations, `${runKey}.candidate.json`)
}

async function readOptionalProductMaterializationState(path, operationKind) {
  ...
  if (!info.isFile() || info.isSymbolicLink()) {
    throw new ClientError('invalid_product_materialization_state', 'Product Materialization recovery state is unsafe.')
  }
  const source = await readBoundedRegularFile(path, {
    errorCode: 'invalid_product_materialization_state',
    maximumBytes: 256 * 1024,
    message: 'Product Materialization recovery state is invalid.',
  })
  let value
  try { value = JSON.parse(source) } catch {
    throw new ClientError('invalid_product_materialization_state', 'Product Materialization recovery state is not valid JSON.')
  }
  validateProductMaterializationState(value, operationKind)
  return value
}

async function writeProductMaterializationState(path, state, operationKind) {
  validateProductMaterializationState(state, operationKind)
...
function validateProductMaterializationState(value, operationKind) {
  ...
  if (state.type !== `porta-product-lifecycle-${operationKind}-operation` || state.version !== 1 ||
    ...
    (state.registrationReceipt !== null && !isRecord(state.registrationReceipt)) ||
    (state.registrationReceipt !== null && state.workReceipt === null)) {
    throw new ClientError('invalid_product_materialization_state', 'Product Materialization recovery state is malformed.')
  }
  if (state.workReceipt !== null) {
    normalizeLocalProductWorkReceipt(state.workReceipt, state, state.workReceipt.traceId)
  }
  if (state.registrationReceipt !== null) {
    normalizeLocalProductRegistrationReceipt(
      state.registrationReceipt,
      state.registrationReceipt.traceId,
    )
  }
  return state
}

function requireSameLocalReleaseIdentity(state, identity) {
  for (const [key, value] of Object.entries(identity)) {
    if (state[key] !== value) {
      throw new ClientError('local_release_operation_conflict', 'Run key is already bound to a different Local Product Release input.')
    }
  }
}
...
async function resolveExistingDirectory(value, label) {
  const path = resolve(String(value ?? ''))
  const actual = await realpath(path).catch(() => undefined)
  const info = actual ? await stat(actual).catch(() => undefined) : undefined
  if (!actual || !info?.isDirectory()) throw new ClientError('invalid_arguments', `${label} must be an existing directory.`)
  return actual
}

async function resolveExistingFile(value, label) {
  const path = resolve(String(value ?? ''))
  const actual = await realpath(path).catch(() => undefined)
  const info = actual ? await lstat(actual).catch(() => undefined) : undefined
  if (!actual || !info?.isFile() || info.isSymbolicLink()) throw new ClientError('invalid_arguments', `${label} must be an existing regular file.`)
  return actual
}
...
async function observeScenePackReadiness(tokens) {
  ...
  writeResult({
    ...
    ok: true,
    receipt,
    type: 'porta-product-lifecycle-client-scene-pack-readiness',
  })
}
...
async function show(tokens) {
  ...
  writeResult({
    ...
    runKey: state.runKey,
    stateFile,
    type: 'porta-product-lifecycle-client-state',
...
async function releaseStatus(tokens) {
  ...
  writeResult({
    ...
    runKey: state.runKey,
    stateFile,
    type: 'porta-product-lifecycle-client-release-status',
    workflowProtocolVersion: RELEASE_WORKFLOW_PROTOCOL_VERSION,
  })
}
...
async function writeManifest(tokens) {
  ...
  writeResult({
    ...
    type: 'porta-product-lifecycle-client-manifest',
  })
}
...
function clientReceiptResult(command, stateFile, receipt, cached) {
  return {
    ...
    receipt,
    stateFile,
    type: 'porta-product-lifecycle-client-receipt',
  }
}
...
function writeError(error) {
  const normalized = error instanceof ProductPackageValidationError ||
    ...
    ? error
    : new ClientError('client_failed', 'Porta Product Lifecycle client failed unexpectedly.')
  process.stderr.write(`${JSON.stringify({
    code: normalized.code,
    ...(normalized.details ? { details: normalized.details } : {}),
    message: normalized.message,
    ok: false,
    type: 'porta-product-lifecycle-client-error',
  }, null, 2)}\n`)
}
