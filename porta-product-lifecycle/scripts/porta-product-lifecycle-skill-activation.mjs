...
const RELEASE_SUBDIRECTORY = 'porta-product-lifecycle'
...
function transactionPaths(parent, operationId) {
  return {
    backup: join(parent, `.porta-product-lifecycle.backup-${operationId}`),
    journal: join(parent, '.porta-product-lifecycle.activation.json'),
    lock: join(parent, '.porta-product-lifecycle.activation.lock'),
    recoveryClaim: join(parent, '.porta-product-lifecycle.activation.recovery'),
    stage: join(parent, `.porta-product-lifecycle.stage-${operationId}`),
  }
}
...
export async function activatePortaProductLifecycleSkill(input) {
...
async function main() {
  ...
  const receipt = await activatePortaProductLifecycleSkill({
...
function writeError(error) {
  const normalized = error instanceof ActivationError
    ? error
    : new ActivationError('activation_failed', 'Porta Product Lifecycle Skill activation failed.')
  process.stderr.write(`${JSON.stringify({
    code: normalized.code,
    message: normalized.message,
    ok: false,
    type: 'porta-product-lifecycle-skill-activation-error',
  }, null, 2)}\n`)
}
