# Porta Product Lifecycle Bridge v1
...
Resolve the absolute directory containing `SKILL.md`, then set `CLIENT` conceptually to its `scripts/porta-product-lifecycle.mjs`. Invoke it with Node 18 or newer from the project root.

```bash
node "/absolute/path/to/porta-product-lifecycle/scripts/porta-product-lifecycle.mjs" capabilities
