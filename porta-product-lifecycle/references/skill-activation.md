# Porta Product Lifecycle Skill activation transaction

This reference is only for the Scene Pack installation Agent. Reading or
running it does not activate Porta Product Lifecycle, authorize publication, call
...
keep `/porta-product-lifecycle` clean. The helper refuses a different origin, lightweight
...
```bash
node "$HELPER_CHECKOUT/porta-product-lifecycle/scripts/porta-product-lifecycle-skill-activation.mjs" activate \
  --provider "$PROVIDER" \
...
| --- | --- | --- |
| Codex | `$CODEX_HOME` or `~/.codex` | `<home>/skills/porta-product-lifecycle` |
| Claude Code | `$CLAUDE_CONFIG_DIR` or `~/.claude` | `<home>/skills/porta-product-lifecycle` |
| Gemini CLI | `~/.gemini` | `<home>/skills/porta-product-lifecycle` |
...
Provider discovered Porta Product Lifecycle after the exact tree activation. Provider
