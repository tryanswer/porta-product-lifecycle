...
async function createRealBridgeFixture() {
  const root = await mkdtemp(join(tmpdir(), 'porta-product-lifecycle-real-bridge-'))
  const project = join(root, 'project')
  const bridge = join(root, 'porta-bridge')
...
async function writeScenePackReadinessSpec(fixture, name, overrides = {}) {
  const base = {
    ...
    catalogId: 'porta-product-lifecycle',
    installedSkills: [{ id: 'porta-product-lifecycle', path: 'porta-product-lifecycle' }],
    provider: 'codex',
    providerDiscovery: 'observed',
    readiness: 'ready',
    release: {
      commitSha: 'e9a5f27ef036f2dcc16f8b73dea2e53c913d76c8',
      tag: 'porta-product-lifecycle-v0.1.1',
      version: '0.1.1',
    },
...
test('begin persists exact Bridge identity and replays locally without a duplicate WorkRun', async () => {
  ...
  try {
    ...
    assert.equal(result.receipt.skillVersion, '1.0.6')
...
test('new identity refuses legacy Skill state instead of emitting a mixed-identity WorkRun', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillId = 'porta-workflow'
    ...
    assert.equal(JSON.parse(rejected.stderr).code, 'invalid_client_state')
    const calls = (await readFile(fixture.log, 'utf8')).trim().split('\n').map(JSON.parse)
...
test('1.0.6 client resumes a retained 1.0.0 run with its original Bridge identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillVersion = '1.0.0'
    await writeFile(result.stateFile, JSON.stringify(state))
    ...
    assert.equal(resumed.receipt.skillVersion, '1.0.0')
    const calls = (await readFile(fixture.log, 'utf8')).trim().split('\n').map(JSON.parse)
    const beginCalls = calls.filter((call) => call[1] === 'begin')
    assert.equal(beginCalls.length, 2)
    assert.equal(beginCalls[1][beginCalls[1].indexOf('--skill-version') + 1], '1.0.0')
  } finally {
    await fixture.cleanup()
  }
})

test('1.0.6 client resumes a retained 1.0.1 run with its original Bridge identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillVersion = '1.0.1'
    await writeFile(result.stateFile, JSON.stringify(state))
    ...
    assert.equal(resumed.receipt.skillVersion, '1.0.1')
    const calls = (await readFile(fixture.log, 'utf8')).trim().split('\n').map(JSON.parse)
    const beginCalls = calls.filter((call) => call[1] === 'begin')
    assert.equal(beginCalls.length, 2)
    assert.equal(beginCalls[1][beginCalls[1].indexOf('--skill-version') + 1], '1.0.1')
  } finally {
    await fixture.cleanup()
  }
})

test('1.0.6 client resumes a retained 1.0.2 run with its original Bridge identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillVersion = '1.0.2'
    await writeFile(result.stateFile, JSON.stringify(state))
    ...
    assert.equal(resumed.receipt.skillVersion, '1.0.2')
    const calls = (await readFile(fixture.log, 'utf8')).trim().split('\n').map(JSON.parse)
    const beginCalls = calls.filter((call) => call[1] === 'begin')
    assert.equal(beginCalls.length, 2)
    assert.equal(beginCalls[1][beginCalls[1].indexOf('--skill-version') + 1], '1.0.2')
  } finally {
    await fixture.cleanup()
  }
})

test('1.0.6 client resumes a retained 1.0.3 run with its original Bridge identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
...
test('1.0.6 client resumes a retained 1.0.4 run with its original Bridge identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillVersion = '1.0.4'
    await writeFile(result.stateFile, JSON.stringify(state))
    ...
    assert.equal(resumed.receipt.skillVersion, '1.0.4')
  } finally {
    await fixture.cleanup()
  }
})

test('1.0.6 client refuses an invented prior version of the new identity', async () => {
  const fixture = await createFixture()
  try {
    const { key, result } = await beginReleaseRun(fixture)
    const state = JSON.parse(await readFile(result.stateFile, 'utf8'))
    delete state.receipt
    state.skillVersion = '0.9.0'
    await writeFile(result.stateFile, JSON.stringify(state))
...
test('Scene Pack readiness is neutral, deterministic, replayable, and never begins a WorkRun', async () => {
  ...
  try {
    ...
    assert.equal(first.type, 'porta-product-lifecycle-client-scene-pack-readiness')
...
test('Scene Pack readiness accepts only coherent Agent claims and terminal readiness states', async () => {
  ...
  try {
    const invalidCases = [
      ...
      { installedSkills: [{ id: 'porta-product-lifecycle', path: '../outside' }] },
      { unexpected: 'field' },
    ]
