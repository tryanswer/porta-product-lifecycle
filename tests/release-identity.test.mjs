...
test('development identity and latest immutable release are internally explicit', async () => {
  ...
  assert.match(readme, /immutable release `porta-product-lifecycle-v1\.1\.0`/u)
  assert.match(readme, /git clone --branch porta-product-lifecycle-v1\.1\.0/u)
  assert.match(readme, /Lifecycle `1\.1\.0` adds deterministic/u)
